...
export class AddColorAndSizeToOrder1705071206178 implements MigrationInterface {
  name = 'AddColorAndSizeToOrder1705071206178';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`order\` ADD \`size\` enum ('M', 'L', 'XL', 'S', 'XXL') NULL DEFAULT 'M'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`order\` ADD \`color\` enum ('#ffffff', '#808080', '#333333', '#ff0005', '#ff8c00', 'Green', 'Red', 'White', 'Blue', 'Orange', 'Black', 'Grey', 'Brown', 'Pink', 'Purple', 'Ash') NULL`,
    );
    await queryRunner.query(
