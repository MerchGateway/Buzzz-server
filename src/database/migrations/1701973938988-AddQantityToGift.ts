...
export class AddQantityToGift1701973938988 implements MigrationInterface {
  name = 'AddQantityToGift1701973938988';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`product\` (\`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL, \`thumbnail\` text NULL, \`in_stock\` tinyint NOT NULL DEFAULT 1, \`price\` decimal NOT NULL, \`description\` varchar(255) NULL, \`is_published\` tinyint NOT NULL DEFAULT 0, \`is_public\` tinyint NOT NULL DEFAULT 1, \`receipt_id\` varchar(255) NULL, \`purchased\` tinyint NOT NULL DEFAULT 0, \`bio\` varchar(255) NULL, \`category_id\` varchar(36) NULL, \`seller_id\` varchar(36) NULL, \`design_id\` varchar(36) NULL, UNIQUE INDEX \`REL_3d12e5ac2924935dd903ef576b\` (\`design_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`transaction\` (\`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`id\` varchar(36) NOT NULL, \`reference\` varchar(255) NOT NULL, \`amount\` decimal(10,2) NOT NULL, \`currency\` enum ('NGN') NULL, \`message\` varchar(255) NULL, \`method\` enum ('DEBIT', 'CREDIT') NOT NULL, \`status\` enum ('FAILED', 'SUCCESS', 'PENDING') NOT NULL DEFAULT 'PENDING', \`type\` enum ('Personal', 'Gift', 'Pay-Gift-Forward') NOT NULL DEFAULT 'Personal', \`channel\` enum ('CARD', 'BANK', 'USSD', 'QR', 'MOBILE_MONEY', 'BANK_TRANSFER') NOT NULL, \`fee_amount\` decimal(10,2) NOT NULL DEFAULT '0.00', \`is_hidden\` tinyint NOT NULL DEFAULT 0, \`transfer_code\` varchar(255) NULL, \`wallet_id\` varchar(36) NULL, \`fee_id\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`order\` (\`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`id\` varchar(36) NOT NULL, \`seller_id\` varchar(255) NOT NULL, \`quantity\` decimal NOT NULL, \`type\` enum ('Personal', 'Gift', 'Pay-Gift-Forward') NOT NULL DEFAULT 'Personal', \`total\` decimal(10,2) NOT NULL, \`shipping_details\` text NULL, \`polymailer_details\` text NULL, \`delivery_fee\` decimal(10) NULL DEFAULT '0', \`coupon\` varchar(255) NULL, \`status\` enum ('Pending', 'Paid', 'Cancelled', 'In-Progress', 'Printed', 'Sent-For-Delievery', 'Delievered', 'Not-Delievered', 'Completed') NOT NULL DEFAULT 'Pending', \`user_id\` varchar(36) NULL, \`cart_id\` varchar(36) NULL, \`product_id\` varchar(36) NULL, \`printing_partner_id\` varchar(36) NULL, \`logistics_partner_id\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`user\` (\`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`id\` varchar(36) NOT NULL, \`identity_provider\` enum ('GOOGLE', 'TWITTER') NULL, \`identity_provider_id\` varchar(255) NULL, \`email\` varchar(255) NOT NULL, \`first_name\` varchar(255) NULL DEFAULT '', \`last_name\` varchar(255) NULL DEFAULT '', \`password\` varchar(255) NULL, \`pin\` varchar(255) NULL, \`role\` enum ('admin', 'super-admin', 'printing-admin', 'logistic-admin', 'publisher', 'user') NOT NULL DEFAULT 'user', \`bio\` varchar(255) NULL, \`phone_number\` varchar(255) NULL, \`address\` varchar(255) NULL, \`shipping_address\` text NULL, \`is_public\` tinyint NOT NULL DEFAULT 1, \`allow_notification\` tinyint NOT NULL DEFAULT 0, \`registration_token\` varchar(255) NULL, \`allow_two_factor_authentication\` tinyint NOT NULL DEFAULT 0, \`is_two_factor_verified\` tinyint NOT NULL DEFAULT 0, \`two_factor_type\` enum ('GOOGLE', 'IN_APP') NOT NULL DEFAULT 'GOOGLE', \`show_email\` tinyint NOT NULL DEFAULT 1, \`instagram\` varchar(255) NULL, \`username\` varchar(255) NOT NULL, \`facebook\` varchar(255) NULL, \`twitter\` varchar(255) NULL, \`reddit\` varchar(255) NULL, \`email_verification_token\` varchar(255) NULL, \`email_verified\` tinyint NOT NULL DEFAULT 0, \`socket_id\` varchar(255) NULL, \`printing_partner\` varchar(36) NULL, \`logistics_partner\` varchar(36) NULL, \`wallet_id\` varchar(36) NULL, UNIQUE INDEX \`IDX_8b068ccc9f2d780253f5d7eae3\` (\`identity_provider_id\`), UNIQUE INDEX \`IDX_e12875dfb3b1d92d7d7c5377e2\` (\`email\`), UNIQUE INDEX \`IDX_858c4058a370d9385b5a182e23\` (\`registration_token\`), UNIQUE INDEX \`IDX_78a916df40e02a9deb1c4b75ed\` (\`username\`), UNIQUE INDEX \`IDX_54663aeef9987efe0b4a3bda93\` (\`email_verification_token\`), UNIQUE INDEX \`REL_b453ec3d9d579f6b9699be98be\` (\`wallet_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`customer\` (\`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`id\` varchar(36) NOT NULL, \`status\` enum ('enabled', 'disabled') NOT NULL DEFAULT 'enabled', \`seller_id\` varchar(36) NULL, \`buyer_id\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`gift\` (\`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`id\` varchar(36) NOT NULL, \`gift_code\` varchar(255) NOT NULL, \`recievers\` text NOT NULL, \`note\` varchar(255) NOT NULL, \`quantity\` int NOT NULL, \`product_id\` varchar(36) NULL, \`order_id\` varchar(36) NULL, UNIQUE INDEX \`IDX_dd5fe96f8bf1c8f3e817feda55\` (\`gift_code\`), UNIQUE INDEX \`REL_9b531b99ffe61e202a283a4bd9\` (\`order_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`waitlist\` (\`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`id\` varchar(36) NOT NULL, \`client\` varchar(255) NOT NULL, UNIQUE INDEX \`IDX_ba0b1c6d4785ad308ef8a0e910\` (\`client\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX \`IDX_ba0b1c6d4785ad308ef8a0e910\` ON \`waitlist\``,
    );
    ...
    await queryRunner.query(
      `DROP INDEX \`REL_9b531b99ffe61e202a283a4bd9\` ON \`gift\``,
    );
    await queryRunner.query(
      `DROP INDEX \`IDX_dd5fe96f8bf1c8f3e817feda55\` ON \`gift\``,
    );
