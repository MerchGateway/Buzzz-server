...
export class CreateWaitListDBAndCreateGiftDBAndUpdatedOrderTable1700236061269
  implements MigrationInterface
{
  name = 'CreateWaitListDBAndCreateGiftDBAndUpdatedOrderTable1700236061269';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`gift\` (\`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`id\` varchar(36) NOT NULL, \`gift_code\` varchar(255) NOT NULL, \`recievers\` text NOT NULL, \`product_id\` varchar(36) NULL, \`order_id\` varchar(36) NULL, UNIQUE INDEX \`IDX_dd5fe96f8bf1c8f3e817feda55\` (\`gift_code\`), UNIQUE INDEX \`REL_9b531b99ffe61e202a283a4bd9\` (\`order_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`transaction\` ADD \`type\` enum ('Personal', 'Gift', 'Pay-Gift-Forward') NOT NULL DEFAULT 'Personal'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`order\` ADD \`type\` enum ('Personal', 'Gift', 'Pay-Gift-Forward') NOT NULL DEFAULT 'Personal'`,
    );
    await queryRunner.query(
